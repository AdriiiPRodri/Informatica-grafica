...
void animarIndependiente(void){
    escena->animarIndependiente();
}
...
int main(int argc, char **argv) {
    ...
    std::vector<caras> triangulos;
    std::string nombre, nombre2;

    if(argc == 1){
		std::cout << "Especifica un fichero PLY de entrada si quieres tener un modelo en el boton 4." << std::endl;
	}
    else if(argc == 2){
    ...
    escena = new Escena(nombre, nombre2);
    ...
    glutCreateWindow("Práctica 1: Adrián Jesús Peña Rodríguez");
    ...
    // funcion para animacion por defecto
    glutIdleFunc(animarIndependiente);
